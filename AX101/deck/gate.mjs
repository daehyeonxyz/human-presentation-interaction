...
const probe = `(() => { const s = document.querySelector('.s.active'); const st = document.getElementById('stage').getBoundingClientRect(); const k = st.width / 1920; const out = { clip: [], blue: [] };
  ...
  s.querySelectorAll('*').forEach(el => { const cs = getComputedStyle(el); if (cs.color !== 'rgb(20, 40, 160)' || cs.visibility === 'hidden') return; for (let q = el; q && q !== s; q = q.parentElement) if (getComputedStyle(q).opacity === '0') return; if (!el.childNodes.length || ![...el.childNodes].some(nd => nd.nodeType === 3 && nd.textContent.trim())) return; if (el.closest('.eye, .foot, .no, .sel, .hot, .lit, .on, .bx, .k, .st, .pj, .menu, .brief, .g, .w.new, .tabs')) return; out.blue.push(el.className.toString().slice(0, 30) + ':' + el.textContent.trim().slice(0, 14)); });
  return out; })()`;
for (let p = 1; p <= n; p++) {

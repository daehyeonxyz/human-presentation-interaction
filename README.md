...
이 레포가 스킬의 본체이자 개발 이력이다. 루트의 스킬 파일이 배포 단위이고, `demos/` 와 `ax-education/` 과 `AX101/` 과 `qa/` 는 그 스킬을 실전으로 검증한 기록이다. 실전에서 받은 지적은 프로젝트 폴더의 피드백 기록에 전수로 모이고, 되풀이된 것만 루트의 스킬 파일로 올라간다.
...
| `ax-education/` | 실전 발표 프로젝트 1호. 디자인 시스템과 원고와 산출물의 실물 표본 |
| `AX101/` | 실전 발표 프로젝트 2호. 제품 재현이 화면의 절반인 43쪽 덱. `08-피드백-전수.md` 가 받은 지적과 승격처의 대조표다 |
